...
class MobyGamesCompanySearch(BaseScraper):
    ...
    def _build_url(self) -> str:
        ...
        return f"{self.base_url}/search/companies?q={self.query}&p={self.page_number}"

    def search_companies(self) -> Dict:
        url = self._build_url()
        response = self._make_request(url)

        # Parse search results
        search_results = response.xpath(Companies.SEARCH_RESULTS)
        results = []

        for result in search_results:
            name = self._get_text(result.xpath(Companies.COMPANY_NAME))
            url_path = self._get_text(result.xpath(Companies.COMPANY_URL))
            company_id = self._extract_id_from_url(url_path)

            games_count_text = self._get_text(result.xpath(Companies.COMPANY_GAMES_COUNT))

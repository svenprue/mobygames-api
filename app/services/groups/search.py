...
class MobyGamesGroupSearch(BaseScraper):
    ...
    def _build_url(self) -> str:
        ...
        return f"{self.base_url}/search/groups?q={self.query}&p={self.page_number}"
    ...
    def search_groups(self) -> Dict:
        ...
        search_results = response.xpath(Groups.SEARCH_RESULTS)
        results = []

        for result in search_results:
            name = self._get_text(result.xpath(Groups.GROUP_NAME))
            url_path = self._get_text(result.xpath(Groups.GROUP_URL))
            group_id = self._extract_id_from_url(url_path)

            details = self._get_text(result.xpath(Groups.GROUP_DETAILS))
